...
void ABoard::CheckLine()
{
    ...
    while (z < 20)
    {
        ...
        bool b = GetWorld()->OverlapMultiByChannel(OutOverlaps,
            Location, Rotation.Quaternion(), ECollisionChannel::ECC_WorldDynamic,
            CollisionShape, Params, ResponseParam);
        if (!b || OutOverlaps.Num() < 9)  // Se cambia el numero de bloques necesarios para eliminar 
        {
            ++z;
            continue;
        }
...
void ABoard::Check_Line()
{
    ...
    while (z < 20)
    {
        ...
        bool b = GetWorld()->OverlapMultiByChannel(OutOverlaps,
            Location, Rotation.Quaternion(), ECollisionChannel::ECC_WorldDynamic,
            CollisionShape, Params, ResponseParam);
        if (!b || OutOverlaps.Num() < 9)  //Se cambia el número de colisiones entre bloques necesarios para eliminar la fila 
        {
            ++z;
            continue;
        }
